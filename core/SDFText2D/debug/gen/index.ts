...
import { gen as genTinySDFCanvas2D, glyphsCache } from "./TinySDF.Canvas2D";
import { gen as genTinySDFWebGL2 } from "../gen/TinySDF.WebGL2";

export const initialization = () => {
  const TinySDFCanvas2DCanvas = genTinySDFCanvas2D(tinySdfInstance, "Hello World!");
  const e1 = document.createElement("div");
  e1.appendChild(TinySDFCanvas2DCanvas);
  document.body.appendChild(e1);

  const TinySDFWebGL2Canvas = genTinySDFWebGL2(tinySdfInstance, "Hello World!");
  ...
  console.warn("glyphsCache", glyphsCache);
};
