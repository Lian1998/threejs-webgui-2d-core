...
void main() {
  ...
  if (uUseShadow == 1.0) {
    ...
    float totalStep = uShadowArray[0] + uShadowArray[1];
    float blackStep = uShadowArray[0] / totalStep;
    // float whiteStep = uShadowArray[1] / totalStep;
    ...
    // 归一化到 [0,1) 的周期位置
    float phase = mod(proj, totalStep) / totalStep;
